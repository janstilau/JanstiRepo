...
ApplicationWindow {
    ...
    function onNotificationPosted(noti) {
        console.log(noti);
    }

    Component.onCompleted: {
        mIdMaker.color = Qt.rgba(0,180,120, 255);
        mIdMaker.setAlgorithm(ColorMaker.LinearIncrease);
        console.log(mIdMaker.algorithm())
        mIdMaker.start()

        notiCenter.addObserver(root, "onNotificationPosted", "AAA");
    }

    Connections {
        target: mIdMaker;
        onCurrentTime:{
            root.title.text = strTime;
