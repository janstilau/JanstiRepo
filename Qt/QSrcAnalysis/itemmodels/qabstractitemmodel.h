...
// 从这里可以看出, ModelIndex 就是存储了原始 model, 以及几个 row, column 的值而恶意.
class Q_CORE_EXPORT QModelIndex
{
    friend class QAbstractItemModel;
public:
     inline QModelIndex()  : row(-1), column(-1), id(0), sourceModel(nullptr) {}
    // compiler-generated copy/move ctors/assignment operators are fine!
     inline int row() const  { return row; }
     inline int column() const  { return column; }
     inline quintptr internalId() const  { return id; }

    inline void *internalPointer() const  { return reinterpret_cast<void*>(id); }
    ...
    inline const QAbstractItemModel *model() const  { return sourceModel; }
    inline bool isValid() const  { return (row >= 0) && (column >= 0) && (sourceModel != nullptr); }
    inline bool operator==(const QModelIndex &other) const
        { return (other.row == row) && (other.id == id) && (other.column == column) && (other.sourceModel == sourceModel); }
    inline bool operator!=(const QModelIndex &other) const
        { return !(*this == other); }
private:
    inline QModelIndex(int arow, int acolumn, void *ptr, const QAbstractItemModel *amodel)
        : row(arow), column(acolumn), id(reinterpret_cast<quintptr>(ptr)), sourceModel(amodel) {}
     inline QModelIndex(int arow, int acolumn, quintptr id, const QAbstractItemModel *amodel)
        : row(arow), column(acolumn), id(id), sourceModel(amodel) {}
    int row, column;
    quintptr id;
...
{ return sourceModel ? sourceModel->parent(*this) : QModelIndex(); }

inline QModelIndex QModelIndex::sibling(int arow, int acolumn) const
{ return sourceModel ? (row == arow && column == acolumn) ? *this : sourceModel->sibling(arow, acolumn, *this) : QModelIndex(); }

inline QModelIndex QModelIndex::siblingAtColumn(int acolumn) const
{ return sourceModel ? (column == acolumn) ? *this : sourceModel->sibling(row, acolumn, *this) : QModelIndex(); }

inline QModelIndex QModelIndex::siblingAtRow(int arow) const
{ return sourceModel ? (row == arow) ? *this : sourceModel->sibling(arow, column, *this) : QModelIndex(); }
...
{ return sourceModel ? sourceModel->data(*this, arole) : QVariant(); }

inline Qt::ItemFlags QModelIndex::flags() const
{ return sourceModel ? sourceModel->flags(*this) : Qt::ItemFlags(); }
