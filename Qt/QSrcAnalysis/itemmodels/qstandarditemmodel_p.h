...
public:
    ...
    QStandardItemModel *model; // 存储真正的数据
    QStandardItem *parent; // 父对象.
    QVector<QStandardItemData> values;
    QVector<QStandardItem*> children; // 子对象
    int rows; // 几行
    int columns; // 几列
...
public:
    ...
    QVector<QStandardItem*> columnHeaderItems; // 表头数据
    QVector<QStandardItem*> rowHeaderItems; // 表头数据
    QScopedPointer<QStandardItem> root; // 根数据
