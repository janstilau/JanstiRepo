...
/*!
    一个集中的清理工作. 和适当的时刻, 调用这样的一个清理方法. 这个时刻可能很多, 例如初始化失败, 重启服务等等.
    所以, 有很多时机要进行清理, 而不单单在析构函数里面.
*/
...
bool QProcessPrivate::_q_canWrite()
{
    ...
    const bool writeSucceeded = writeToStdin(); // 将缓存里面的数据, 写入到标准输入中.
...
{
    Q_D(QProcess);
    if (d->processState == state) // 相等判断, 应该当做是一个标准写法来记忆.
        return;
    d->processState = state;
    // 只有在这里, 有着 stateChanged 信号的发送.
...
//! 真正的进程的启动, 在这里首先做一些状态的重置工作, 然后进行进程的启动.
//! 这里的状态, 是在父进程里面, 用来控制, 或者查询新创建进程的状态而做的一些工作.
//! 为了保持这些状态, 父子进程要做通信工作.
