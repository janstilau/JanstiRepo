    ...
    if (channel.type == Channel::Normal) {
        ...
        if (threadData->hasEventDispatcher()) {
            if (&channel == &stdinChannel) {
                ...
                // 当 channel.notifier 发送了 activated 之后, 就进行相应的逻辑判断. 这是代码里面主动进行后续逻辑触发的办法.
                QObject::connect(channel.notifier, SIGNAL(activated(int)),
                                 q, receiver);
            }
        }
...
void QProcessPrivate::startProcess()
{
    ...
    // 这里, 如果是 MAC OS, 那么可执行文件, 要进去 bundle 里面查找.
    ...
    //! 在这里, 使用了 fork 新生成一个进程. 在这里, 还没有设置新进程的路径.
    pid_t childPid;
    ...
    // Start the child.
    // 在这里, 使用新进程的路径, 进行代码的替换. scope 里面, 是子进程的代码, 所以最后是 exit
    if (forkfd == FFD_CHILD_PROCESS) {
...
void QProcessPrivate::execChild(const char *workingDir, char **path, char **argv, char **envp)
{
    ...
    // 如果, qt_safe_execve 能够执行成功, 就不会执行下面的代码了. 所以, 这里的代码很安全.
    // notify failure
...
bool QProcessPrivate::waitForReadyRead(int msecs) // 轮询. 在这里面不断地检测子进程的 stdout 是否有输出.
{
