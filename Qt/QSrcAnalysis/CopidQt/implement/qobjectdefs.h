...
struct Q_CORE_EXPORT QMetaObject
{
    ...
    struct { // private data
        const QMetaObject *superdata; // 父类的元信息
        const QByteArrayData *stringdata; // 元信息的字符化表示方法
        const uint *data; //元信息的真正存储, 数据结构如下.
        /*
    int revision;
    int className;
    ...
    */
        ...
        StaticMetacallFunction static_metacall; // 一个函数指针, 传递QObject*,call 类型, 方法的序号, 以及参数列表.
        // static_metacall 是每个 Moc 文件下都独自生存的, 也就是说, QObject 的动态性, 是依靠 Moc 编译的时候, 生成的这个
        // 各类自相关的 MetaCall 函数. 元信息编程, 可以用一个统一的函数指针进行处理, 但是, 这个函数指针, 每个类都不相同. 编译生成.
