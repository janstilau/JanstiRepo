...
private:
    // MetaMethod 里面存储的东西很少, 元信息对象, 在元信息对象的位置.
    // 因为, 所有的元信息数据, 都在元信息对象里进行的存储.
...
// 成员函数的具体如何从 data 取值, 设置没有细看. 这和 Qt 的具体实现相关.
...
private:
    const QMetaObject *mobj; // Enum 其实也是归属于一个 metaObject 的.
    uint handle;
