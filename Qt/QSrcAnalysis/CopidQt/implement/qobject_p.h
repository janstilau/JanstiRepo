...
public:
    ...
    {
        ExtraData() {}
        QVector<QObjectUserData *> userData; // QObject 可以通过 setUserData(int, value), userData(int) 的形式进行值得关联. 在 Dict Pen 里面使用了这种方式.
        QList<QByteArray> propertyNames; // 动态进行 Property 设置, 就是存到了这里.
        QVector<QVariant> propertyValues; // 动态进行 Property 设置, 就是存到了这里.
        QVector<int> runningTimers; // 记录了一下 自己开启的 timer 的句柄.
...
public:
    ...
    // 将所需要的数据, 按照类型, 放到了不同的对象里面.
    ExtraData *extraData;    // objectName, propertyList, timerid.
    QThreadData *threadData; // 线程相关的信息, 这个线程, 就是 this 所在的线程.
    QObjectConnectionListVector *connectionLists; // Connection 列表.
    Connection *senders;     // connet 到自己的所有发送者
...
private:
    ...
    const QObject *sender_; // 发送者
    int signalId_; // 发送信号.
    int nargs_;
    int *types_;
    void **args_;
    QSemaphore *semaphore_; // 阻塞发送线程使用.
