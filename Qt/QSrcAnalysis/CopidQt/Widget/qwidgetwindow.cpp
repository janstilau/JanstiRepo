...
// 这是一个非常重要的类, 系统的各种事件, 首先会到达这里. 然后在这里, 会将各种系统的事件, 寻找对应的 receiver, 然后在调用相应的 Application 的 notify 函数进行分发操作.
// 应该说, 这里是所有的事件的核心.
...
// 在这里, 除了被点击之外, 还有有其他各种逻辑, 例如被点击窗口重新返回激活状态等等.
void QWidgetWindow::handleMouseEvent(QMouseEvent *event)
{
    ...
    // 在这里, 找到了真正的 receiver, 然后才是 receiver 的 event 函数被调用.
