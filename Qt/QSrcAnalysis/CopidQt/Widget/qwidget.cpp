...
// 之所以 Widget 可以增加 Action, 是因为有右键单击出现菜单的可能.
...
// 如果, 视图有 layout, 那么使用 layout 的 sizeHint 实现. 默认 Widget 返回 invalidSize
...
// 默认, PaintEvent 没有任何的作为. 当控件需要进行自定义显示内容的时候, 把相关的代码放到这里.
