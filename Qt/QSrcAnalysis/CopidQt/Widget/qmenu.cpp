...
void QMenuPrivate::activateCausedStack(const QVector<QPointer<QWidget> > &causedStack, QAction *action, QAction::ActionEvent action_e, bool self)
{
    ...
    if(self){
        action->activate(action_e); // 在这里, 进行了对应的回调的触发.
    }
...
/*!
    各种简便的方法, 只是简化了外界使用的难度. 在内部, 还是使用原来的机制, 生成对应的对象, 使用参数对这些对象进行一系列的设置工作.
*/
...
void QMenu::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    if (action && action == d->currentAction) {
        if (!action->menu()){
          // View 层处理各种事件, 然后在合适的时机, 触发回调.
          // 和 iOS 不同的是, iOS 里面, 是直接将数据层存储的回调, 挂钩到 View 上. 在 View 的事件里面, 触发对应的回调
            // 而 Qt 里面, 触发的函数, 还是在数据层, View 会调用数据层的触发函数. 也就是对应的信号的发送.
