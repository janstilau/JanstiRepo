...
QObjectPrivate::~QObjectPrivate()
{
    if (extraData && !extraData->runningTimers.isEmpty()) {
        if (Q_LIKELY(threadData->thread == QThread::currentThread())) {
            ...
            // 这里就是为什么 QObject 里面, 要把所有自己开启的定时器存储的原因.
            // 它要在析构的时候, 进行删除操作.
...
// 通过 Method 找 Index 的方法, 因为 method 里面本身就存储了 handle, 所以直接利用了 handle 的值.
...
// 用宏的方式, 进行连接的 connect 函数.
// 简单来说, 就是通过参数, 生成一个 Connection, 然后存到 sender 的 connectionList 内部.
                                     ...
                                     Qt::ConnectionType type)
{
    ...
    // 到这里, receiver 回调方法的 index 就获取到了.
    // 然后检查, 信号回调的函数签名和参数个数类型.
...
#endif
    // QMetaObjectPrivate::connect 里面, 会对 connection 进行注册处理.
    QMetaObject::Connection handle = QMetaObject::Connection(QMetaObjectPrivate::connect(
        sender, signal_index, smeta, receiver, method_index_relative, rmeta ,type, types));
    return handle;
}

// 和使用字符串查找的思路没有太大的变化, 仅仅是找 index 的时候的方法发生了变化.
                                                        ...
                                                        int type, int *types)
{
    ...
    // 在这里, 进行真正的 Connenction 的生成工作. 就是把传过来的值存起来.
