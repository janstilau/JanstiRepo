...
// CSmartPtr 里面, 有一个 CRefCount 计数器.
// 构造函数, 创建计数器, 记录资源.
// 拷贝构造, 复制计数器指针, 计数器+1.
// 赋值操作符, 原有计数器 -1. 复制对方计数器, 计数器 + 1
// 析构, 计数器 -1.
// 从现在来看, 使用计数器对象, 和直接在 SmartPtr 里面, 进行一个 int 计数的管理, 没有太大的区别.
template<typename T>
class CSmartPtr
{
public:
