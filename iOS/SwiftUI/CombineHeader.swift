...
/*
这个类型, 就是可以通知外界, 我的数据改变了.
如果不使用 @Published, 其实就是在每一个 Property 改变之后, 手动的调用自己的  objectWillChange.send() 方法, 通知外界这里有数据发生了改变.
而使用了 @Published 之后, 就是每一个 property 的 set 方法里面, 自动调用 objectWillChange.send() 方法.

这是一个类才能实现的协议, 是一个引用协议.
*/
...
public protocol ObservableObject : AnyObject {
    ...
    // 当 model 的属性发生改变的时候, 要主动使用 objectWillChange 对外进行广播.
...
///
/*
    当被 @Published 修饰之后, 属性的 willSet 里面, 会触发 publisher 的 send 方法. 使得 Model 想关联的 View 可以获知, 数据即将发生改变. 
*/
