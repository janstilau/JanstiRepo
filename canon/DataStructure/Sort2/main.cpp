...
/*
 ...
 快排:
 快排的思路是, 先随便取队头或者队尾的一个值做标准, 然后将这个数组前半部分变为小于等于这个值得空间, 后半部分是大于这个值得空间, 这个值在它最终排序的位置, 然后递归, 快排前半部分, 快排后半部分.
 快排的这个分区间的过程, 并不需要两个区间是有序的, 只要保证两个区间是分局这个标准进行了区分.
 在这个算法里面, 如果我们用队首元素, 它会在 partion 的程序里面跑到中间位置, 所以, 这个算法不是稳定的排序算法.
 ...
 我们前面讲过，归并之所以是非原地排序算法， 主要原因是合并函数无法在原地执行。快速排序通过设计巧妙的原地分区函数，可以实现原地排 序，解决了归并排序占用太多内存的问题。
 */
