...
/*
 如果用笔沿着二叉树进行描绘, 那么其实每一个节点, 都有着三次的访问的机会. 第一次是在左从上往下走的时候, 第二次是在遍历完左子树回到自己位置的时候, 第三次遍历完右子树对到自己位置的时候. 在不同的时机, 添加访问操作, 这就是前中后遍历.
 */
...
/*
 层次遍历
 层次遍历要用到数组, 先取根节点, 入队. 然后出队, 在访问节点前, 或者后的时候, 将这个节点的左右子树入队. 然后第一层就完了. 这个时候, 队内有着第二层的两个节点. 然后出队, 继续上面的节奏. 这个时候 ,第二层第一个节点的两个子节点会入队. 这样循环下去, 每次下一层节点要访问的时候, 上一层节点一定是按照顺序访问完了. 等到队列为空的时候, 也就是遍历完成的时候.
 */
