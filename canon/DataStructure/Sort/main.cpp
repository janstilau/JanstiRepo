...
/*
 ...
 排序的空间复杂度
 in-place 排序, 指的是在原有空间上进行排序, 不引入新的数组或者链表进行存储. 例如, 冒泡, 选择, 都是inplace 排序
 空间复杂度临时存储空间是可以的, 但是如果要根据 n 进行缓存的存储, 那么就应该算到空间复杂度里面了.
 
 稳定性.
 稳定的排序算法是, 在排序过后, 原有的相同的两个值, 前后顺序保持不变.
 年龄排序是我们的第二标准, 在年龄相同的时候, 通过积分排序. 在积分排序的过程中, 要保持原有的顺序不变的.
 如果年龄相同的两个人, 根据积分排序之后的顺序发生了改变, 这很有可能导致年龄排序的结果被破坏了. 所以, 一定要准求排序的稳定性.
 ...
 就和查找一样, 查找的过程, 如果一次操作能够大量的筛选掉不符合的 item 的话, 那么这个查找的效率就高, 而排序的过程, 如果一次操作能够大量的消除逆序对的话, 那么这个排序的效率就高.
 对于 n*n 的排序算法, 每一次比较, 移动, 只能减少一个逆序对, 而其他的排序算法, 归并在合并两个字序列的过程中, 快排在交换两个位置元素的过程中, 都一次性消除了大量的逆序对, 所以他们的效率就高.
 ...
 插入算法的核心思想是取未排序区间中的元素，在已排序区间中找到合适的插入位置将其插入，在这个过程中, 数组插入会有数据搬移的操作, 插入排序的过程, 就是数组插入的过程.
 重复这个过程，直到未排序区间中元 素为空，算法结束。
 冒泡排序的时间复杂度为 n*n, 主要的操作, 其实是浪费在了移动数组的过程中, 在这个过程中, 移动一次, 就会减除一个逆序对. 所以, 插入排序也是比较慢的排序.
 
 选择排序, 是一种不稳定的算法.
 选择排序, 在移动元素的时候, 一次可以消除大量的逆序对, 但是在寻找最大最小值的过程中, 需要大量的比较操作, 所以, 也是 n*n 的时间复杂度.
 比如 5，8，5，2，9 这样一组数据，使用选择排序算法来排序的话，第一次找到最小元素 2，与第 一个 5 交换位置，那第一个 5 和中间的 5 顺序就变了，所以就不稳定了。正是因此，相对于冒泡排序和插入排序，选择排序就稍微逊色了。
 
 我们前面分析冒泡排序和插入排序的时候讲到，冒泡排序不管怎么优化，元素交换的次数是一个固定值，是原始数据的逆序度。插入排序是同样的，不管怎么优化，元素移动的次数也等于原始数据 的逆序度。
...
// 每次循环, 找到应该调换的角标. 大部分都是做判断操作, n 次值的交换操作.
/*
 选择排序, 是下意识中最简单的排序, 但是由于不稳定性, 之后还是少用该算法.
 */
