...
/*
 ...
 capture 里面收集数据, 回到到 session, 然后 session 交给 encoder 进行处理, encoder 处理完之后, 回调到 session, session 交给 socket 进行 sendFrame 的操作.
 这么看来, session 的作用还是协调者.
 
 感觉这个类, 很好的解释了. AVCaptureSession 的工作. 从 input 中取得数据, 然后解码输出到 output 中去.
...
// 下面的四个对象, 是通过懒加载的方式建立起来的. 并且将代理设置给 self
