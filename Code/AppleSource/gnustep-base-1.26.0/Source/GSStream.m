...
- (void) _sendEvent: (NSStreamEvent)event
{
    ...
    else if (event == NSStreamEventOpenCompleted)
    {
        if ((_events & event) == 0)
        {
            _events |= NSStreamEventOpenCompleted; // 在这里, 更改事件的状态. 并且通知代理.
    ...
    else if (event == NSStreamEventHasSpaceAvailable)
    {
        ...
        if ((_events & NSStreamEventHasSpaceAvailable) == 0)
        {
            ...
            {
                [_delegate stream: self
                      handleEvent: NSStreamEventHasSpaceAvailable]; // 通知代理.
            }
        }
    }
    else if (event == NSStreamEventErrorOccurred)
    {
        if ((_events & NSStreamEventErrorOccurred) == 0)
        {
            ...
            {
                [_delegate stream: self
                      handleEvent: NSStreamEventErrorOccurred]; // 通知代理.
            }
        }
    }
    else if (event == NSStreamEventEndEncountered)
    {
        if ((_events & NSStreamEventEndEncountered) == 0)
        {
            ...
            {
                [_delegate stream: self
                      handleEvent: NSStreamEventEndEncountered]; // 通知代理.
            }
        }
    }
    else
...
- (void) _unschedule
{
    ...
    enumerator = NSEnumerateMapTable(_loops);
    while (NSNextMapEnumeratorPair(&enumerator, (void **)(&aRunLoop), (void**)&modes))
    {
        unsigned	i = [modes count];
        
        while (i-- > 0)
        {
            [aRunLoop removeStream: self mode: [modes objectAtIndex: i]];
        }
    }
    NSEndMapTableEnumeration(&enumerator);
}
...
// 这个的调用, 应该是在Stream 的delegate 里面, 而不是一个while 循环中
- (NSInteger) read: (uint8_t *)buffer maxLength: (NSUInteger)len
{
    ...
    if (len + _pointer >= dataSize) // 修正 length 的值.
    {
