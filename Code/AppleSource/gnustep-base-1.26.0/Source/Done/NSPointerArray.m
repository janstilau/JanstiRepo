...
// 在内部创建配置类的对象, 最终还是靠这个配置类的对象进行初始化工作.
...
- (NSArray*) allObjects
{
    ...
    else
    {
        ...
        // 再次循环一遍, 将里面的值添加到 NSArray 中, 这个时候, 里面的值就被retain一次了
        for (i = 0; i < _count; i++)
        {
...
// 就是简单的取出 NULL, 这里是记录两个指针, 后面的遍历覆盖前面的.
- (void) compact
{
    NSUInteger	insert = 0;
    NSUInteger	i;
...
- (void) insertPointer: (void*)pointer atIndex: (NSUInteger)index
{
    ...
    i = _count;
    [self setCount: _count + 1]; // 这里面会有扩容给的操作.
    while (i > index)
    {
        pointerFunctionsMove(&_pf, _contents+i, _contents + i-1);
        i--;
    }
    pointerFunctionsAcquire(&_pf, &_contents[index], pointer); // 这里, 会根据内存管理的策略进行赋值操作.
}
