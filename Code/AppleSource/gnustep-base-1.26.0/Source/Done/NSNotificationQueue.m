...
currentList(void)
{
    ...
    threadDict = GSCurrentThreadDictionary();
    list = (NotificationQueueList*)[threadDict objectForKey: lkey];
    if (list == nil)
    {
        list = [NotificationQueueList new];
        [threadDict setObject: list forKey: lkey];
    }
    return list;
}
...
+ (NSNotificationQueue*) defaultQueue
{
    ...
    list = currentList(); // 根据 thread 获取到 list.
    item = list->queue;
    if (item == nil)
    {
...
- (id) init
{
    return [self initWithNotificationCenter:
            [NSNotificationCenter defaultCenter]]; // 默认就是 defaultcenter.
}
                    ...
                    forModes: (NSArray*)modes
{
    ...
    if (coalesceMask != NSNotificationNoCoalescing)
    {
        [self dequeueNotificationsMatching: notification
                              coalesceMask: coalesceMask]; // 这里, 就是根据coalesce 的策略, 提前进行删除操作.
    }
    switch (postingStyle)
    {
        case NSPostNow: // 如果是同步的, 那么立马就进行 post 操作.
        {
            NSString	*mode;
       ...
       NSString *mode, NSZone *zone)
{
    ...
    if (len > 0)
    {
        ...
        for (pos = 0; pos < len; pos++)
        {
            ...
            [center postNotification: n]; // 发送通知.
            RELEASE(n);
        }
