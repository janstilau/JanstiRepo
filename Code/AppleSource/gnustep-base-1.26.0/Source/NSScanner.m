...
- (id) initWithString: (NSString *)aString
{
    ...
    _string = RETAIN(aString);    c = object_getClass(_string);
...
- (BOOL) isAtEnd
{
    ...
    ret = !skipToNextField(); // 这里, 这个宏改变了 变量的位置, 所以在这里进行了存储工作.
    _scanLocation = save__scanLocation;
    return ret;
}

/*
 和编程题的策略没有什么区别. 所以, 编程题就是这些商业代码的实现.
 */
- (BOOL) _scanInt: (int*)value
{
    ...
    // 首先, 检测符号, 然后设置到状态位. 更改 scanLocation
    if (_scanLocation < myLength())
    {
    ...
    while (_scanLocation < myLength())
    {
        ...
        if ((digit < '0') || (digit > '9')) // 非法值, 不是数值.
            break;
        if (!overflow)
        {
            if (num >= limit) // 如果在乘以 10 , 就越界了.
                overflow = YES;
            else
                num = num * 10 + (digit - '0'); // 累加.
        }
        _scanLocation++;
        got_digits = YES;
    }
