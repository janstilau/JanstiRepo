...
public struct UnsafePointer<Pointee>: _Pointer {
    ...
    /*
     就是内存地址值, 一定要存下这个值, 各个方法, 就是在这个值的基础上, 增加了类型的限制.
     */
    ...
    /*
     deinitialized
     如果, pointee 的数据, 是一个对象, 那么一定要调用 deinitialized 函数. deinitialized 里面, 会调用对象的析构方法.
     deallocate 就是自己的内存的释放, 对象所管理的资源, 一定是对象的析构函数里面进行管理.
     这个和 delete 会先调用析构, 后进行 free 一样的.
     所以, Swfit 可以用大量的 C++ 的概念去理解.
     
     一定要分得清, allocated, initilized, deallocted, 和 uninitlized 的分别.
     */
    @inlinable
    ...
    public func withMemoryRebound<T, Result>(to type: T.Type, capacity count: Int,
        ...
        // 这里, defer 就是在函数结束之后, 把 _rawValue 绑定回去.
        defer {
...
public struct UnsafeMutablePointer<Pointee>: _Pointer {
    ...
    @discardableResult
    /*
     就是调用所指向的对象的析构函数, 这是为了释放所指向的对象所管理的资源.
     */
