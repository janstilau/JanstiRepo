...
/*
    sorted 代表着返回一个新的数据, 这个数据, 是原有数据排序之后的拷贝.
 */
...
/*
 对于序列来说, 只能调用 sorted, 不能再原有位置进行排序.
 但是对于集合来说, 因为集合的概念, 就是位置存储, 所以可以在原有位置进行排序.
 */
...
extension MutableCollection where Self: RandomAccessCollection {
    ...
    public mutating func sort(
        ...
        /*
         _withUnsafeMutableBufferPointerIfSupported 到底是做什么的, 还是需要去思考.
         */
        ...
        if didSortUnsafeBuffer == nil {
            ...
            /*
             indices, sortedElements.indices 必定是 count 相等的.
             indices, sortedElements.indices 本身就是带次序的. 因为索引在这里面已经排好序了, 所以遍历操作, 可以保证 self 的有序.
...
extension UnsafeMutableBufferPointer {
    ...
    /*
     这个算法, 是在 BufferPointer 上的. 也就是说, 只要能够提供这样一个 Buffer, 他就能够排序. buffer 应该提供 count 的大小.
     */
