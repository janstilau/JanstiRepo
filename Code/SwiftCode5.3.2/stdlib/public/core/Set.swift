...
// _Variant 其实就是一个抽象层, 他把 Set 需要的操作, 都定义了, 在内部, 在根据当前的 type, 分发到不同的真实的数据对象上.
...
// 所有的 set 对于 Collection 的适配, 都是转交给了 _variant, _variant 实现了抽象的所有方法.
...
// 从这里可以看出, _variant 有两种, native 和 cocoa.
...
extension Set: SetAlgebra {
    ...
    // 之所以有这个方法, 是以为, 两个对象可能会不同, 但是 hash 值不同, 那么可以用新的值进行替换.
    ...
    where Source.Element == Element {
        ...
        // 如果, sequence 本身就是 set, 那么直接就是 _variant 的替换. 否则, 就是一个个的插入.
