...
// 从这里我们看出, 同样的一个 Element, 在不同的 set 里面, hashValue 是不一样的. 因为 seed 是根据容器来变化的.
extension _NativeSet { // Low-level lookup operations
    ...
    // 这里, 可以看出, Swift 原生的哈希表, 没有使用链表法
    internal func find( _ element: Element, hashValue: Int) -> (bucket: Bucket, found: Bool) {
...
extension _NativeSet.Iterator: IteratorProtocol {
    internal mutating func next() -> Element? {
        // 直接使用 iterator, hash 表内部, 会进行迭代数据的改变.
