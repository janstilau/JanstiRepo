...
internal struct _StringObject {
    ...
    internal enum Variant {
        case immortal(UInt) // 32
        case native(AnyObject) // 64
    ...
    internal var _countAndFlagsBits: UInt64 // 标识部分, UInt 64, 8 个字节.
    internal var _object: Builtin.BridgeObject // 指针部分. 8 个字节
...
extension _StringObject {
    ...
    internal init(object: AnyObject, discriminator: UInt64, countAndFlags: CountAndFlags
    ) {
    ...
    internal var discriminatedObjectRawBits: UInt64 {
        ...
        #else
        // 如果是 SmallObject 进行的初始化, 那么 SmallObject 一定会保证, 标志位是 true.
...
extension _StringObject {
    ...
    internal var isSmall: Bool {
        // 判断, 第三个位置是不是 1. 如果是, 那么就是 smallString.
        // 2000 -> 0b 0010 0000 0000 0000 0000 0000 0000 0000
...
/*
 ...
 当, 是 smallString 的时候, 16 个字节, 前面 15 个字节, 存储数据, 最后一个字节, 存储个数.
...
// StringObject 就是 16 个字节, 如果, 是 SmallObjct 进行初始化的时候, 就用 SmallObject 填充
extension _StringObject {
...
/*
 ...
 如果不是 smallObject, 那么有 8 个字节, 是存储下面的这些标识信息, 8 个字节, 存储指针信息.
 ...
 The top 16 bits are for performance flags, which are not
 ...
 count (aka endIndex).
 前面的是一些效率标识符, 后面的是字符串的长度, codeUnit 的 count.
 ┌─────────┬───────┬──────────────────┬─────────────────┬────────┬───────┐
