...
fileprivate func _allocateStringStorage(
    ...
    let (storage, numTailBytes) = _allocate(
        ...
        // Builtin.allocWithTailElems_1 才是真正的分配内存的函数.
...
: __SwiftNativeNSString, _AbstractStringStorage {
    ...
    // asString, as 开头的方法, 在里面生成对应的一个对象.
    final internal var asString: String {
        // 直接把 Storage, 变为了 _StringGuts 的 _object
...
// 根据 capacity 以及各种配置, 生成一块空间, 但是没有初始化.
extension __StringStorage {
    private static func create(
        ...
        let (storage, capAndFlags) = _allocateStringStorage(codeUnitCapacity: capacity)
    ...
    internal static func create(
        ...
        // 生成了空间之后, 按照指针的值, 进行 initialize 的操作.
