// 这里, 返回的是一个数组, 不是 Sequence 了.
// 其实想下也很简单, 没有位置存放排序之后的数据, 一定是在 sort 方法内部生成位置来存放.
// 既然是生成数据来存放, 那就是数组了.
...
// 如果, 是 random 的, 那么可以原地进行排序.
extension MutableCollection where Self: RandomAccessCollection {
    public mutating func sort(
        by areInIncreasingOrder: (Element, Element) throws -> Bool
    ) rethrows {
        // 如果, 可以拿到原始的内存, 可以直接在内存层面上进行排序.
        ...
        if didSortUnsafeBuffer == nil {
            // 不然的话, 先外部排序, 然后进行一次重置操作.
            // 这里, sorted 里面, 先会进行 ContiguousArray 的生成, 所以最终还是到了 buffer._stableSortImpl
