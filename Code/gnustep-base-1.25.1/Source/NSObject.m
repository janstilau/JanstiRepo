...
- (NSMethodSignature*) methodSignatureForSelector: (SEL)aSelector
{
    ...
    if (0 == aSelector) // 防范式编程
    {
        return nil;
    }
    
    objClass = object_getClass(self);
     ...
     */
    if (!class_respondsToSelector(objClass, aSelector))  // 防范式编程
    {
    ...
    // 总之, 上面的操作就是 找 types, 然后通过 types 构建签名. types 里面存的就是 返回值类型, 各个参数的类型.
    if (types == 0)
    {
        return nil;
    }
    return [NSMethodSignature signatureWithObjCTypes: types];
    // 这应该是  NSMethodSignature 的 designated init. 所以其实 NSObject 里面还是用的这个.
}
