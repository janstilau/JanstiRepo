...
{
    // 假设 auth 里面是 Basic realm=KOALUSAP管理工具控制台登录
    if ([auth isKindOfClass: [NSString class]] == YES)
    {
        ...
        scanner = [NSScanner scannerWithString: auth];
        key = [mimeParser scanName: scanner];
        if ([key caseInsensitiveCompare: @"Basic"] == NSOrderedSame)
        {
            method = NSURLAuthenticationMethodHTTPBasic; // 最简单的.
            domain = [URL path];
        }
        ...
        while ((key = [mimeParser scanName: scanner]) != nil)
        {
            if ([scanner scanString: @"=" intoString: 0] == NO)
            {
                return nil;	// Bad name=value specification
            }
            if ((val = [mimeParser scanToken: scanner]) == nil)
            {
                return nil;	// Bad name=value specification
            }
