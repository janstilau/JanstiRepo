...
// 获取 class C 对应的 GSKVOReplacement 的信息, 这里面会有缓存机制.
...
{
    ...
    tmp = [[NSString alloc] initWithCharacters: &u length: 1];
    a[0] = [NSString stringWithFormat: @"set%@%@:", tmp, suffix]; // a[0] == setName
    a[1] = [NSString stringWithFormat: @"_set%@%@:", tmp, suffix]; // a[1] == _setName
    // 为什么, 会有 _setName 这一样, 因为 APPLE 其实是大量的运用了 _作为私有方法的实现.
    for (i = 0; i < 2; i++)
    {
             ...
             context: (void*)aContext
{
     ...
     */
    dot = [aPath rangeOfString:@"."]; // 如果添加的是 keyPath
    if (dot.location != NSNotFound)
    {
