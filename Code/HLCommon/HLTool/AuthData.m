...
{
    //取消别名
    [MiPushSDK unsetAlias:loginUser.uid];
    
    loginUser = nil;
    [self synchronize];
    
    //更新统计模块的注册用户
    [MCClick updateRegisterUser];
    
    [Bugly setUserIdentifier:[AppUDID value]];
    [Bugly setUserValue:@"" forKey:@"nickname"];
}

+ (void)loginSuccess:(NSDictionary *)info
{
//    [[NLCoreData shared] reloadDatabase];
...
//    }
    ...
    [Bugly setUserIdentifier:loginUser.uid];
    [Bugly setUserValue:loginUser.name forKey:@"name"];
    
    [(AppDelegate *)[UIApplication sharedApplication].delegate registerMiPush];
    
    //设置别名
    [MiPushSDK setAlias:loginUser.uid];
