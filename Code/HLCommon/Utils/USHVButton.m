...
@interface USHVButton ()

@property (assign, nonatomic) CGRect last_bounds;
...
- (void)setImageSize:(CGSize)size forState:(UIControlState)state
{
    ...
    if (oldImage && !CGSizeEqualToSize(oldImage.size, size)) {
        [self setImage:[oldImage imageWithSize:size] forState:state];
    }
}
...
- (void)layoutSubviews
{
    ...
    if (!CGRectEqualToRect(self.bounds, self.last_bounds)) {
        [self makeEdgeHighlighted:self.highlighted];
    }
}
