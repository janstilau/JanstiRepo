...
+ (void) initialize
{
    ...
    if (self == [NSImageRep class]) {
        ...
        [imageReps addObject: [NSBitmapImageRep class]]; // 最初, 只有一种
    }
}

// 类簇模式, 对于子类的管理操作.
/**
 * 由一个数组管理所有可以生成的子类对象 , 然后根据这个数据, 完成工厂方法.
 */
+ (Class) imageRepClassForData: (NSData *)data
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ([rep canInitWithData: data]) // 不同的子类, 根据 data 的值, 判断自己是否可以根据 data 生成该 image.
            return rep;
    }
    return Nil;
}
