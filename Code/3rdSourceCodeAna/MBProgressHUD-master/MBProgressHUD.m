...
- (void)commonInit { // 在两个 designated 方法里面调用这个, 不过, 有必要为一个一定不会序列化的类写出 initWithCoder 码.
...
- (void)setProgress:(float)progress { // 重画.
    if (progress != _progress) {
        _progress = progress;
        [self setNeedsDisplay];
    }
...
- (void)setProgressColor:(UIColor *)progressColor { // 重画.
...
- (void)setProgressRemainingColor:(UIColor *)progressRemainingColor { // 重画.
