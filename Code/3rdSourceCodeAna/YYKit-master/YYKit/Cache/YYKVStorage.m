...
// 所有的 db 相关的方法, 都是在类的内部的. 对外暴露出去的方法, 要包装成为了各个相关的sql语句.
...
- (BOOL)_dbInitialize {
    ...
    extended_data blob,\
    primary key(key)); \
    create index if not exists last_access_time_idx on manifest(last_access_time);"; // 这里, 因为是LRU的实现, 所以用了last_access_time创建了索引.
