...
// 这里就是 alloc init 的意义所在, 所有的初始化方法归结到一点, 子类只需要重新, 或者调用这一点就可以了.
...
- (void)_SR_commonInit;
{
    ...
    // 这里, delegate 默认的 queue 是主线程的 queue
...
{
    // dispatch_get_specific 获取当前 queue 的 key 对应的值. 这里主要是为了 当前 queeue 是不是就是 workQueue
...
- (void)open;
{
    ...
    _selfRetain = self; // 强引用, 这里有循环引用, 所以不会消失.

    if (_urlRequest.timeoutInterval > 0) // 如果设置了连接超时的限制, 那么超过时间之后, 就会链接失败的方法, 在那个方法里面, 会调用代理方法, 并且停止链接.
    {
...
// 在 delegate Queue 中执行回调, 这里觉得设计的不好, 为什么要引入两种机制. 设计者强制用一种显得更加的方便. 可能是为了客户, 因为客户在业务代码里面, 可能会用不同的方法.
...
// 这里, 更改了 queue, 就是更改了 _delegateDispatchQueue 的值.
...
- (void)_initializeStreams;
{
    ...
    //可以看出, webSocket 在底层还是用的 socket 实现的.
    ...
    _outputStream = CFBridgingRelease(writeStream);// 转换内存控制权, 之后就由对象进行控制.
    _inputStream = CFBridgingRelease(readStream);// 转换内存控制权, 之后就由对象进行控制.
...
{
    dispatch_async(_workQueue, ^{//在 workQueue 中执行.
...
{    
    ...
    if (_closeWhenFinishedWriting) { // 这个值会在 closeConnection 中设置为 YES.
            return;
    }
...
- (void)_pumpWriting;
{
    ...
    // A boolean value that indicates whether the receiver can be written to. : hasSpaceAvailable
    // _outputBufferOffset 表示已经写入数据的偏移量, 这里是说, 如果有新数据了, 并且还有空间, 那就写入.
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        NSInteger bytesWritten = [_outputStream write:_outputBuffer.bytes + _outputBufferOffset maxLength:dataLength - _outputBufferOffset];
        if (bytesWritten == -1) { // 写入失败, 直接关闭连接, 并且报告给代理.
...
- (BOOL)_innerPumpScanner {
    ...
    if (consumer.consumer) {
        ...
        foundSize = consumer.consumer(tempView);// 这里, 把对于 data 的使用, 封装到了 consumer 里面. 所以这里是取值, 然后交给了 consumer 进行处理.
    } else {
...
-(void)_pumpScanner;
{
    ...
    if (!_isPumping) {
        _isPumping = YES; // 这里, 看来这种设置一个值然后在方法末尾重新设置的方式经常用啊.
    } else {
        return;
    }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    ...
    if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) { // 安全相关的, 先没看.
