...
#define GSI_MAP_CLEAR_VAL(node)  
    ...
    struct	_GSIMapNode {
        ...
        GSIMapKey	key; // key, 用来查找,
#if	GSI_MAP_HAS_VALUE
        GSIMapVal	value; // value, 用来存储.
#endif
    };
...
#define GSI_MAP_TABLE_S sizeof(GSI_MAP_TABLE_T)
    ...
    GSIMapMoreNodes(GSIMapTable map, unsigned required)
    {
        ...
        if (newArray)
        {
            ...
            newNodes
            = (GSIMapNode)NSZoneCalloc(map->zone, chunkCount, sizeof(GSIMapNode_Pointer));
            if (newNodes)
            {
    ...
    GSIMapSize(GSIMapTable map)
    {
         ...
         */
        size += map->bucketCount * sizeof(GSIMapBucket_Pointer);
        
        /* Add the free nodes.
         */
        for (node = map->freeNodes; 0 != node; node = node->nextInBucket)
        {
            size += sizeof(GSIMapNode_Pointer);
        }
        ...
        {
            size += sizeof(GSIMapNode_Pointer) * map->buckets[index].nodeCount;
        }
        return size;
    }
