...
/**
 * <p>This describes an informal protocol for <em>key-value coding</em>,
 *
 * kvc 的到底是什么的描述.
 * a mechanism whereby the fields of an object may be accessed and set using
 * generic methods in conjunction with string keys rather than field-specific
 * methods.
 * kvc 的代价.
 *  Key-based access loses compile-time validity checking, but can be
 * convenient in certain kinds of situations.</p>
 *
 * 子类可以复写 KVC , 来完成自己的一些操作.
