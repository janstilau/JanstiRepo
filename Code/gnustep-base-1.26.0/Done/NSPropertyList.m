...
@implementation GSXMLPListParser // 可以看到, XML 的解析, 也是用到了 NSXMLParser 的功能.
  ...
  qualifiedName: (NSString *)qName
{
    ...
    if (inContainer)
    {
        ...
        {
            ASSIGN(result, [stack lastObject]);
        }
        else
        {
    ...
    else if ([elementName isEqualToString: @"integer"])
    {
        if ([value hasPrefix: @"-"])
        {
            ASSIGN(result, [NSNumber numberWithLongLong: [value longLongValue]]);
        }
        else
        {
            ASSIGN(result, [NSNumber numberWithUnsignedLongLong:
                           (unsigned long long)[value longLongValue]]);
        }
    }
    else if ([elementName isEqualToString: @"real"])
    {
        ASSIGN(result, [NSNumber numberWithDouble: [value doubleValue]]);
    }
    ...
    if (inArray == YES)
    {
        [[stack lastObject] addObject: result];
    }
    else if (inDictionary == YES)
    {
        ...
        [(NSMutableDictionary*)[stack lastObject] setObject: result forKey: key];
        DESTROY(key);
    }
    [value setString: @""];
}
...
- (BOOL) parse
{
    if (parsed == NO)
    {
        ...
        success = [theParser parse]; // 这个过程太过于复杂, 地图编辑器的归档解档的过程更加容易理解, 因为那个过程, 是建立在 XML 的代理的基础上的. 具体的过程, 应该和 JSON 是一样的.
    }
    return success;
}
...
encodeBase64(NSData *source, NSMutableData *dest)
{
    ...
    if (length > 0)
    {
        ...
        GSPrivateEncodeBase64((const uint8_t*)[source bytes],
                              length,
                              (uint8_t*)[dest mutableBytes] + base);
    }
}
...
/*
 输出字符串到文本文件里面, 其中做了转义字符和 unicode 字符的转化工作.
 */
static void
XString(NSString* source, NSMutableData *output)
{
    ...
    // 如果, 原文本里面有 xml 的转义字符, 就要在输出的时候, 输出这些转义字符.
    if ([source rangeOfCharacterFromSet: xmlQuotables].length > 0)
    {
        unichar	*base;
        unichar	*XMLresult;
        unichar	currentChar;
        ...
        [source getCharacters: base];
        // 首先, 把输出字符串的长度寻找出来.
        for (resultLength = index = 0; index < end; index++)
        {
            currentChar = base[index];
            switch (currentChar) // 可以看到, swift 里面也可以有 if, else 的判断, 也就是 case 里面前面做特定的判断, defalut 里面, 做范围的判断.
            {
                case '&':
                    resultLength += 5;
                    break;
                ...
                default:
                    if ((currentChar < 0x20 && (currentChar != 0x09 && currentChar != 0x0A && currentChar != 0x0D)) ||
                        (currentChar > 0xD7FF && currentChar < 0xE000) ||
                        currentChar > 0xFFFD)
                    {
                        if (osx)
                        {
                            resultLength += 8;	// Illegal in XML
                        }
                        else
                        {
                            resultLength += 6;	// Non-standard escape
                        }
                    }
                    else
                    {
        ...
        XMLresult = NSAllocateCollectable(sizeof(unichar) * resultLength, 0);
        for (resultIndex = index = 0; index < end; index++)
        {
            currentChar = base[index];
            switch (currentChar)
            {
                case '&':
                    XMLresult[resultIndex++] = '&';
                    XMLresult[resultIndex++] = 'a';
                    XMLresult[resultIndex++] = 'm';
                    XMLresult[resultIndex++] = 'p';
                    XMLresult[resultIndex++] = ';';
                    break;
                case '<':
                    XMLresult[resultIndex++] = '&';
                    XMLresult[resultIndex++] = 'l';
                    XMLresult[resultIndex++] = 't';
                    XMLresult[resultIndex++] = ';';
                    break;
                case '>':
                    XMLresult[resultIndex++] = '&';
                    XMLresult[resultIndex++] = 'g';
                    XMLresult[resultIndex++] = 't';
                    XMLresult[resultIndex++] = ';';
                    break;
                case '\'':
                    ...
                    XMLresult[resultIndex++] = 'a';
                    XMLresult[resultIndex++] = 'p';
                    XMLresult[resultIndex++] = 'o';
                    XMLresult[resultIndex++] = 's';
                    XMLresult[resultIndex++] = ';';
                    break;
                case '"':
                    XMLresult[resultIndex++] = '&';
                    XMLresult[resultIndex++] = 'q';
                    XMLresult[resultIndex++] = 'u';
                    XMLresult[resultIndex++] = 'o';
                    XMLresult[resultIndex++] = 't';
                    XMLresult[resultIndex++] = ';';
                    break;
                ...
                default:
                    // 后面的, 就用统一的字符转换.
                    if ((currentChar < 0x20 && (currentChar != 0x09 && currentChar != 0x0A && currentChar != 0x0D))
                        || (currentChar > 0xD7FF && currentChar < 0xE000) || currentChar > 0xFFFD)
                    {
                        if (osx)
                        {
                            ...
                            XMLresult[resultIndex++] = '&';
                            XMLresult[resultIndex++] = '#';
                            XMLresult[resultIndex++] = 'x';
                            XMLresult[resultIndex++] = hexdigits[(currentChar>>12) & 0xf];
                            XMLresult[resultIndex++] = hexdigits[(currentChar>>8) & 0xf];
                            XMLresult[resultIndex++] = hexdigits[(currentChar>>4) & 0xf];
                            XMLresult[resultIndex++] = hexdigits[currentChar & 0xf];
                            XMLresult[resultIndex++] = ';';
                        }
                        else
                        {
                    ...
                    {
                        XMLresult[resultIndex++] = currentChar;
                    }
                    break;
            }
        }
        NSZoneFree(NSDefaultMallocZone(), base);
        source = [[NSString alloc] initWithCharacters: XMLresult length: resultLength];
        NSZoneFree(NSDefaultMallocZone(), XMLresult);
        [output appendData: [source dataUsingEncoding: NSUTF8StringEncoding]];
        RELEASE(source);
    }
    else
    {
 ...
 * dest is the output buffer.
 最终的生成 Plist 文件的方法. 和 JSON 很像, 就是按照类型, 和固定的规则, 输出到文本文件中.
 */
static void
OAppend(id source, NSDictionary *loc, unsigned lev, unsigned step,
        NSPropertyListFormat x, NSMutableData *dest)
{
    ...
    // Plist 文件里面, 只允许做固定类的输出工作, 所以, 这里固定做了类型的判断.
    if ([source isKindOfClass: NSStringClass])
    {
        if (x == NSPropertyListXMLFormat_AppleUsed)
        {// 可以看到, Plist 用前缀后缀进行了类型的输出.
        ...
        {
            PString(source, dest); // PString 没看.
        }
    }
    else if (source == boolY) // BOOL 固定输出 true
    {
        if (x == NSPropertyListXMLFormat_AppleUsed)
        {
        ...
        {
            PString([source description], dest);
        }
    }
    else if (source == boolN) // BOOL 固定输出 false
    {
        if (x == NSPropertyListXMLFormat_AppleUsed)
        {
    ...
    else if ([source isKindOfClass: NSNumberClass])
    {
        const char	*t = [source objCType];
        ...
        if (strchr("cCsSiIlLqQ", *t) != 0)
        {
            if (x == NSPropertyListXMLFormat_AppleUsed)
            {
                ...
                XString([source stringValue], dest);
                [dest appendBytes: "</integer>\n" length: 11];
            }
            else if (x == NSPropertyListGNUstepFormat) // 数值, 用类型进行包装.
            {
                [dest appendBytes: "<*I" length: 3];
                [dest appendData:
                 [[source stringValue] dataUsingEncoding: NSASCIIStringEncoding]];
    ...
    else if ([source isKindOfClass: NSDataClass]) // Data 进行 base64 的输出.
    {
        if (NSPropertyListXMLFormat_AppleUsed == x)
        {
            [dest appendBytes: "<data>\n" length: 7];
            encodeBase64(source, dest);
            [dest appendBytes: "</data>\n" length: 8];
        }
...
#define num2char(num) ((num) < 0xa ? ((num)+'0') : ((num)+0x57))
    ...
    else if ([source isKindOfClass: NSDateClass]) // 时间, 就固定的年月日时分秒的输出.
    {
        ...
        if (x == NSPropertyListXMLFormat_AppleUsed)
        {
            ...
            source = [source descriptionWithCalendarFormat: @"%Y-%m-%dT%H:%M:%SZ"
                                            timeZone: z locale: nil];
            source = [source dataUsingEncoding: NSASCIIStringEncoding];
            [dest appendData: source];
            [dest appendBytes: "</date>\n" length: 8];
        }
        else if (x == NSPropertyListGNUstepFormat)
        {
            [dest appendBytes: "<*D" length: 3];
            source = [source descriptionWithCalendarFormat: @"%Y-%m-%d %H:%M:%S %z"
                                            timeZone: z locale: nil];
            source = [source dataUsingEncoding: NSASCIIStringEncoding];
            [dest appendData: source];
            [dest appendBytes: ">" length: 1];
    ...
    else if ([source isKindOfClass: NSArrayClass])
    {
        ...
        if (x == NSPropertyListXMLFormat_AppleUsed) // 如果是数组, 就先输出[, 最后输出], 然后中间调用递归调用 OAppend 方法.
        {
            NSEnumerator	*e;
            ...
            while ((source = [e nextObject]))
            {
                ...
                OAppend(source, loc, level, step, x, dest);
            }
            [dest appendBytes: iBaseString length: strlen(iBaseString)];
            [dest appendBytes: "</array>\n" length: 9];
        }
    }
    else if ([source isKindOfClass: NSDictionaryClass])
    {
        const char	*iBaseString;
        const char	*iSizeString;
        SEL		objSel = @selector(objectForKey:);
        IMP		myObj = [source methodForSelector: objSel];
        unsigned		i;
        NSArray		*keyArray = [source allKeys];
        ...
        if (isProxy == YES)
        {
            for (i = 0; i < numKeys; i++)
            {
                ...
                plists[i] = [(NSDictionary*)source objectForKey: keys[i]];
            }
        }
        else
        {
            ...
            for (i = 0; i < numKeys; i++)
            {
                plists[i] = (*myObj)(source, objSel, keys[i]);
            }
        }
    ...
    else
    {
        ...
        {
            [dest appendBytes: "<string>" length: 8];
            XString([source description], dest);
...
+ (NSData *)dataWithPropertyList: (id)aPropertyList
                          ...
                          options: (NSPropertyListWriteOptions)anOption
                            error: (out NSError**)ouputError // 这里面根本没有对于 error 的操作.
{
    NSMutableData	*dest;
    NSDictionary	*loc;
    int		step = 2;
    ...
    if (aFormat == NSPropertyListXMLFormat_AppleUsed)
    {
...
// 真正的生成 propertyList 的方法, 最后的数据, 会被塞到 str 中
void
GSPropertyListMake(id obj, NSDictionary *loc, BOOL xml,
                   BOOL forDescription, unsigned step, NSMutableString* *outputStr)
{
    NSString		*tmp;
    NSPropertyListFormat	style;
    
    if (classInitialized == NO) // 首先, 做一些初始化的工作, 这个在OC 里面, 可以写到 initialize 方法里面.
    {
        [NSPropertyListSerialization class];
    }
    
    // 这些不同的输出格式, 使得最后输出的时候, 会有不同的字符串变化, 相应的, 解析的时候, 也要反处理这些. 应该在输出的字符串中, 会标明自己的形式. 例如, 放在头几个字节, 或者直接有一个字符串进行存储.
    if (forDescription)
    {
    ...
    {
        style = NSPropertyListXMLFormat_AppleUsed; // 苹果的 plist 文件, 就是用这个存储的.
    }
    ...
    if (style == NSPropertyListXMLFormat_AppleUsed)
    {
        [dest appendBytes: prefix length: strlen(prefix)]; // 苹果的 plist 文件, 固定有着这样的前缀.
                      ...
                      error: (out NSError**)error
{
    ...
    if (errorStr == nil) // 这里, 需要根据不同的 format, 用不同的类进行解析的工作
    {
        switch (format)
        {
            case NSPropertyListXMLFormat_AppleUsed:
            {
