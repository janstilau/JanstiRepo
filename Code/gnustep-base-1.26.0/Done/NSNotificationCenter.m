...
static Observation *
obsNew(NCTable *table, SEL selector, id observer)
{
    Observation	*obs;
    if (table->freeList == 0)
    {
    ...
    obs = table->freeList;
    table->freeList = (Observation*)obs->link;
    obs->link = (void*)table;
    obs->retained = 0;
    obs->next = 0;
...
@implementation NSNotificationCenter // 所有的通知的总的管理者.
...
+ (void) initialize
{
    ...
    {
        default_center = [[self alloc] init];
    }
}
