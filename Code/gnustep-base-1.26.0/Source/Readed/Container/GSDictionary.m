...
/*
 * GSDictionary is not the end. It is just a adapater for GSIMapTable_t.
 GSImapTable is a hashMap.
 Key and value is one entry node. So they are in the same object. The progress of finding is find the node.
 1. get key's hash code to find bucket.
 2. isEqual judge every item in the bucket to find the item.
 
 GSIMapTable_t is so nosiy. So I don't spend time to read it. The hash map principle is general.
 */
                 ...
                 count: (NSUInteger)c
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        // get node form map through key.
        node = GSIMapNodeForKey(&map, (GSIMapKey)(id)keys[i]);
        if (node)
        {
            // get node means there was keyvalue entry cached in the map.
            // OCDictionary is not a multiply map, so the latter value will replace the former
            IF_NO_GC(RETAIN(objs[i]));// retain the new value
            RELEASE(node->value.obj);// release the old value
            node->value.obj = objs[i];// node replace the old value with new value
        }
        else
        {
            GSIMapAddPair(&map, (GSIMapKey)(id)keys[i], (GSIMapVal)objs[i]);
            // insert a new node to map. Node is inited with new key and value.
        }
    }
    return self;
}
                ...
                copyItems: (BOOL)shouldCopy
{
    ...
    if (c > 0)
    {
        ...
        for (i = 0; i < c; i++)
        {
            ...
            k = [k copyWithZone: z]; // key must by copied.
            if (k == nil)
            {
                DESTROY(self);
                [NSException raise: NSInvalidArgumentException
            ...
            if (shouldCopy)
            {
                o = [o copyWithZone: z]; // value can be copy
            }
            else
            {
                o = RETAIN(o); // value can be retain.
            }
            if (o == nil)
            {
                DESTROY(self);
...
- (id) objectForKey: aKey
{
    if (aKey != nil)
    {
        GSIMapNode	node  = GSIMapNodeForKey(&map, (GSIMapKey)aKey); // get node firstly.
        
        if (node)
        {
            return node->value.obj; // return node. value instance value
        }
    }
    return nil;
}
...
{
    [super init];
    dictionary = (GSDictionary*)RETAIN(d); // retain the dict.
