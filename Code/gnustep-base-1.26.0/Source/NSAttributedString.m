...
+ (void) initialize
{
    if (self == [NSAttributedString class])
    {
        ...
        getAttributesSEL = @selector(attributesAtIndex:effectiveRange:);
                            ...
                            inRange: (NSRange)rangeLimit
{
    ...
    IMP getImp = [self methodForSelector: getAttributesSEL];
    attrDictionary = (*getImp)(self, getAttributesSEL, index, aRange);
    if (aRange == 0) return attrDictionary;
    
    while (aRange->location > rangeLimit.location)
    {
        //Check extend range backwards
        tmpDictionary = (*getImp)(self, getAttributesSEL, aRange->location-1, &tmpRange);
    ...
    while (NSMaxRange(*aRange) < NSMaxRange(rangeLimit))
    {
        ...
        tmpDictionary = (*getImp)(self, getAttributesSEL, NSMaxRange(*aRange), &tmpRange);
        if ([tmpDictionary isEqualToDictionary: attrDictionary])
        {
         ...
         inRange: (NSRange)rangeLimit
{
    ...
    getImp = [self methodForSelector: getAttributesSEL];
...
- (void) addAttribute: (NSString*)name value: (id)value range: (NSRange)aRange
{
    ...
    getImp = [self methodForSelector: getAttributesSEL];
    attrDict = (*getImp)(self, getAttributesSEL, aRange.location, &effectiveRange);
    ...
    if (effectiveRange.location < NSMaxRange(aRange))
    {
        ...
        while (effectiveRange.location < NSMaxRange(aRange))
        {
            ...
            {
                attrDict = (*getImp)(self, getAttributesSEL, NSMaxRange(effectiveRange),
                                     &effectiveRange);
            }
        }
        [self endEditing];
             ...
             withAttributedString: (NSAttributedString*)attributedString
{
    ...
    if (max > 0)
    {
        ...
        getImp = [attributedString methodForSelector: getAttributesSEL];
        setImp = [self methodForSelector: setSel];
        while (loc < max)
        {
            NSRange	ownRange;
            
            attrDict = (*getImp)(attributedString, getAttributesSEL, loc, &effectiveRange);
