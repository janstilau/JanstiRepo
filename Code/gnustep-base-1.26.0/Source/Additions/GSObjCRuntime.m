    ...
    else
    {
        ...
        switch (*type)
        {
            ...
            case _C_CHR:
            {
                ...
                /**
                 * All primitive is adapted into class value.
                 */
 ...
 * value in an object either via an accessor method
 (if sel is supplied), or
  via direct access (if type, size, and offset are supplied).<br />
 * Automatic conversion between NSNumber and C scalar types is performed.<br />
 * If type is null and can't be determined from the selector, the
...
GSObjCSetVal(NSObject *self, const char *key, id val, SEL sel,
             const char *valueType, unsigned size, int offset)
{
    ...
    // 0 for self, 1 for _cmd, 2 for value.
    if (sel != 0)
    {
        ...
        // Here type is for the value
        valueType = [signature getArgumentTypeAtIndex: 2];
    }
    if (valueType == NULL)
    {
        // Here key is not found.
        [self setValue: val forUndefinedKey:
         [NSString stringWithUTF8String: key]];
    }
    else if ((val == nil || val == null) && *valueType != _C_ID && *valueType != _C_CLASS)
    {
    ...
    else
    {
        ...
        /**
         * For all tyep. Fist using selector, if not, direct set memory value.
         */
        switch (*valueType)
        {
            case _C_ID:
            case _C_CLASS:
            {
                ...
                if (sel == 0)
                {
                    ...
                    // ASSIGN is macor, inside it there is a retain invoke.
                    ASSIGN(*ptr, v);
                }
                else
                {
            ...
            case _C_STRUCT_B:
                // For built-in strcut, this is a special case
                if (GSSelectorTypesMatch(@encode(NSPoint), valueType))
                {
                ...
                else if (GSSelectorTypesMatch(@encode(NSRange), valueType))
                {
                ...
                else if (GSSelectorTypesMatch(@encode(NSRect), valueType))
                {
                ...
                else if (GSSelectorTypesMatch(@encode(NSSize), valueType))
                {
                ...
                else
                {
                    ...
                    NSGetSizeAndAlignment(valueType, &size, 0);
                    if (sel == 0)
                    {
