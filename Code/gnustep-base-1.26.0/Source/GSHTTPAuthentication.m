                                                ...
                                                requestURL: (NSURL*)URL;
{
    if ([auth isKindOfClass: [NSString class]] == YES)
    {
        ...
        authMethod = [mimeParser scanName: sc];
        if ([authMethod caseInsensitiveCompare: @"Basic"] == NSOrderedSame)
        {
            method = NSURLAuthenticationMethodHTTPBasic;
            domain = [URL path];
        }
        else if ([authMethod caseInsensitiveCompare: @"Digest"] == NSOrderedSame)
        {
            method = NSURLAuthenticationMethodHTTPDigest;
        }
        else if ([authMethod caseInsensitiveCompare: @"NTLM"] == NSOrderedSame)
        {
            method = NSURLAuthenticationMethodNTLM;
        }
        else if ([authMethod caseInsensitiveCompare: @"Negotiate"] == NSOrderedSame)
        {
        ...
        while ((authMethod = [mimeParser scanName: sc]) != nil)
        {
            ...
            if ([authMethod caseInsensitiveCompare: @"domain"] == NSOrderedSame)
            {
                domain = val;
            }
            else if ([authMethod caseInsensitiveCompare: @"realm"] == NSOrderedSame)
            {
                realm = val;
            }
            if ([sc scanString: @"," intoString: 0] == NO)
