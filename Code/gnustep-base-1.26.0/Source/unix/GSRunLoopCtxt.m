...
{
    RELEASE(mode);
    GSIArrayEmpty(modeRelatedPerformers);
    NSZoneFree(modeRelatedPerformers->zone, (void*)modeRelatedPerformers);
 ...
 */
- (void) endEvent: (void*)data for: (GSRunLoopWatcher*)watcher
{
    if (completed == NO)
    {
...
- (id) initWithMode: (NSString*)theMode extra: (void*)e
{
    ...
    if (self != nil)
    {
        ...
        extra = e;
        z = [self zone];
        modeRelatedPerformers = NSZoneMalloc(z, sizeof(GSIArray_t));
        ...
        GSIArrayInitWithZoneAndCapacity(modeRelatedPerformers, z, 8);
...
- (BOOL) pollUntil: (int)milliseconds within: (NSArray*)contexts
{
    GSThreadRelatedTaskContainer   *threadInfos = GSThreadCacheTasks(nil);
    ...
    {
        GSRunLoopWatcher	*aWatcher;
        int		fd;
        BOOL		trigger;
        
        aWatcher = GSIArrayItemAtIndex(watchers, watcherCount).obj;
        if (aWatcher->_invalidated == YES)
        {
            GSIArrayRemoveItemAtIndex(watchers, watcherCount);
        } else if ([aWatcher runLoopShouldBlock: &trigger] == NO) {
            if (trigger == YES)
            {
                immediate = YES;
                GSIArrayAddItem(_trigger, (GSIArrayItem)(id)aWatcher);
            }
        } else {
            switch (aWatcher->type)
            {
                case ET_EDESC:
                    fd = (int)(intptr_t)aWatcher->data;
                    if (fd > fdEnd)
                        fdEnd = fd;
                    FD_SET (fd, &exception_fds);
                    NSMapInsert(_efdMap, (void*)(intptr_t)fd, aWatcher);
                    break;
                ...
                case ET_RDESC:
                    ...
                    if (fd > fdEnd)
                        fdEnd = fd;
                    FD_SET (fd, &read_fds);
                    NSMapInsert(_rfdMap, (void*)(intptr_t)fd, aWatcher);
                    break;
                ...
                case ET_WDESC:
                    ...
                    if (fd > fdEnd)
                        fdEnd = fd;
                    FD_SET (fd, &write_fds);
                    NSMapInsert(_wfdMap, (void*)(intptr_t)fd, aWatcher);
                    break;
                    
                case ET_RPORT:
                {
                    id port = aWatcher->receiver;
                    NSInteger port_fd_size = FDCOUNT;
                    NSInteger port_fd_count = FDCOUNT;
                    NSInteger port_fd_buffer[FDCOUNT];
                    NSInteger *port_fd_array = port_fd_buffer;
    ...
    while (completed == NO)
    {
        ...
        if (FD_ISSET (fdIndex, &read_fds))
        {
            ...
            if (fdIndex == threadInfos->inputFd)
            {
                [threadInfos threadRelatedTaskContainerFire];
                watcher = nil;
            }
            else
            {
