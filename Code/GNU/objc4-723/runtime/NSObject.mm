...
/*
 原始的 set 方法, 新值 retain, 旧值 release
 */
void
...
/*
 如果是 tagPointer, 也就是没有真实的存储空间, 直接返回.
 */
id
...
// 直接返回类对象记录的 superClass 的信息, 直接是值的读取.
+ (Class)superclass {
    return self->superclass;
}

// 先找到类对象, 然后读取类对象的 superclass 信息.
- (Class)superclass {
...
// 最终会调用到了 lookUpImpOrForward 方法内部.
// lookUpImpOrForward 中, 会对类进行 initialize 的操作.
+ (BOOL)instancesRespondToSelector:(SEL)sel {
    if (!sel) return NO;
    return class_respondsToSelector(self, sel);
}

// 类方法的 respondsToSelector, 就是去元类对象里面, 查找元类对象里面, 有没有对应的 method_t
...
/*
 从当前类到根类捋一遍, protocol 是否添加到类的声明中.
 */
...
/*
 默认的 HASH, 就是地址的 hash.
 这也就是为什么 OC 里面, 对象可以做 key 值的原因, 根类默认提供了 hash 函数.
 */
+ (NSUInteger)hash {
