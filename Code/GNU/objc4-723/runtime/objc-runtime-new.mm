...
/*
 首先, 获取到 ivar 的 offset, 这个是存在 ivar 对象内的
 */
ptrdiff_t
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    /*
     如果, 类还没有进行 realized 的, 那么先要进行 realizeClass 的调用
     realizeClass 可以认为是, 对于类信息进行初始化操作. 这个过程, 会把类对象的信息, 填充到合适的状态.
     */
    if (!cls->isRealized()) {
    ...
    /*
     如果还没有进行 initialize, 就对类进行 initialize 操作.
     */
