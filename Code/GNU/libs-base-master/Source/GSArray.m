...
/*
 GSArray 这个类, 主要的责任是
 内存管理, 以及 primitive method 的实现.
 这和 NSArray 的文档里面规定的是相符的.
 */
...
/*
 Array 的内存管理, 就是一个最原始的指针数组而已. 然后, 每次进行数据的添加删除的时候, 要有相应的内存管理函数的调用.
 */
...
- (NSUInteger) indexOfObject: anObject
{
    ...
    if (_count > 1)
    {
        ...
        imp = (BOOL (*)(id,SEL,id))[anObject methodForSelector: isEqualSel];
        
        for (i = 0; i < _count; i++)
        {
            if ((*imp)(anObject, isEqualSel, _contents_array[i]))
            {
                return i;
            }
        }
...
- (id) objectAtIndex: (NSUInteger)index
{
    ...
    {
        /*
         数组的越界问题, 在容器类中要进行处理, 在相应的位置, 直接报错了.
         */
...
- (void) removeObject: (id)anObject
{
    ...
    if (index > 0)
    {
        ...
        while (index-- > 0)
        {
            if ((*imp)(anObject, isEqualSel, _contents_array[index]) == YES)
            {
