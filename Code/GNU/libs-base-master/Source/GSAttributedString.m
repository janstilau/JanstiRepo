...
/*
 AttributeString 的真正的数据模型, 就是一个 字符串, 还有就是 infoArray 数组, 这个 infoArray 中存储了不同的区间上, 存储的字符属性.
 
 在这个文件里面, 只会实现这些 primitive Method, 而在类簇文件里面, 会根据这些方法, 去实现更加高级的方法.
 */
...
/*
 从 attributedString 中, 提取属性, 添加到 infoArray 里面.
 这个方法, 应该定义到类里面, 这是因为 attributeString 和 mutableAttributeString 被设计成为了两个不同的类, 所以就用 C 函数的方式, 进行了复用.
 */
static void
_setAttributesFrom(
                   ...
                   NSMutableArray *_infoArray)
{
    ...
    /*
     不断的调用 attributesAtIndex: effectiveRange: 这个方法, 更新 range 的范围.
     */
...
{
    unsigned	low, high, attributesCount, middle, nextLoc;
    GSAttrInfo	*middleInfo = nil;
    
    attributesCount = (*cntImp)(_infoArray, countSel);
    high = attributesCount - 1;
    
    if (index >= tmpLength)
    {
        if (index == tmpLength)
        {
            middleInfo = OBJECTAT(high);
            if (foundIndex != 0)
            {
                *foundIndex = high;
            }
            if (aRange != 0)
            {
                aRange->location = middleInfo->loc;
                aRange->length = tmpLength - middleInfo->loc;
            }
            return middleInfo->attrs;
        }
    ...
    {
        middle = (low + high) / 2;
        middleInfo = OBJECTAT(middle);
        if (middleInfo->loc > index)
        {
            high = middle - 1;
        }
        else
        {
            /*
             这里, 必须要拿到 前后两个 info, 然后判断 index 是否在前后两个 info 的 loc 之间, 才能判断是否找到 index 的 attris, 以及确定 length.
             */
            if (middle >= attributesCount - 1)
            {
                nextLoc = tmpLength;
            }
            else
            {
                GSAttrInfo	*inf = OBJECTAT(middle + 1);
                nextLoc = inf->loc;
            }
            if (middleInfo->loc == index || index < nextLoc)
            {
                //Found
                if (aRange != 0)
                {
                    aRange->location = middleInfo->loc;
                    aRange->length = nextLoc - middleInfo->loc;
                }
                if (foundIndex != 0)
                {
                    *foundIndex = middle;
...
- (void) _sanity
{
    ...
    unsigned	c = (*cntImp)(_infoArray, countSel);
                 ...
                 range: (NSRange)range
{
    ...
    SANITY();
    textLength = [_textChars length];
    arraySize = (*cntImp)(_infoArray, countSel);
    beginRangeLoc = range.location;
    afterRangeLoc = NSMaxRange(range);
    if (afterRangeLoc < textLength)
    {
        /*
         * Locate the first range that extends beyond our range.
         */
        attrs = _attributesAtIndexEffectiveRange(
                                                 afterRangeLoc, &effectiveRange, textLength, _infoArray, &arrayIndex);
        if (attrs == attributes)
        {
