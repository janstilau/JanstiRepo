...
static SEL    removeLastObjectSel;
...
/*
 Array 的迭代器, 和一般的迭代器一样, 都是存储了 array 的原始指针, 以及当前迭代位置.
 当前迭代位置, 是和每个迭代器相关的, 每个迭代器有着自己的 position, 多次获取迭代器, 迭代过程独立就是因为 position 的值不同享.
 */
...
- (id) initWithArray: (NSArray*)anArray
{
    ...
    {
        array = anArray;
        RETAIN(array); // 迭代的过程, 需要用到 array, 所以不会被释放.
        pos = 0;
...
- (void) dealloc
{
    RELEASE(array); // 迭代结束, 就可以释放 array 了.
    [super dealloc];
}
...
/*
 反向迭代, 这个过程应该是迭代器控制的, 而不是 Array, 所以根本没有 ReverseArray, 只是有 reverse 迭代器.
 */
...
/*
 这是一个 Gnu Foundation 的设计, 当这个类消亡的时候, 会触发一些清理工作.
 一般来说, 就是清理一些静态共有数据.
 */
+ (void) atExit
{
...
+ (void) initialize
{
    if (self == [NSArray class])
    {
        ...
        removeLastObjectSel = @selector(removeLastObject);
        ...
        NSAllocateObject(GSPlaceholderArrayClass,
                         0,
                         NSDefaultMallocZone());
...
+ (id) allocWithZone: (NSZone*)z
{
    if (self == NSArrayClass)
    {
         ...
         */
        /*
         类簇模式的实现方式, 返回一个特定的对象, 这个特定对象的 init 方法里面, 进行真正的类的对象的生成操作.
         */
...
{
    id na;
    NSUInteger	currentCount = [self count];
    
    /*
     为什么各个工具类, 会报错, 是因为类的设计者, 主动进行了报错处理
     */
    if (anObject == nil) {
        [NSException raise:NSInvalidArgumentException format: @"Attempt to add nil to an array"];
    }
...
/*
 Primitive Method, 由 GSArray, 完成实际的内容.
 */
- (NSUInteger) count
{
 ...
 */
/*
 所有的相等性判断, 都可以遵循相同的逻辑, 显示元信息的判断, 然后是具体数据的判断.
 指针判断
 类型判断
 元信息判断
 具体数据的判断
 这样的流程下来, 前面几个判断, 可以直接过滤掉大部分的数据.
 */
...
/*
 可以看到, NSArray 里面, 掺杂了大量不是 Array 相关的业务逻辑, 这些如果可以放到专门的 extension 里面, 是更加合理的代码组织方式.
 */
/**
...
/*
 排序相关的操作, 如果可以有 sort Extension, 可以专门放到独立的分区里面.
 */
 ...
 */
// 根据 Sel 进行排序. 利用了 OC 的动态性, 在排序的时候, 进行消息发送找到最终的比较函数.
 ...
 */
// 根据 Block 进行排序, 在排序的时候, 直接调用 Block 进行比较函数的调用.
...
/*
 所有返回一个 sortedArray 的方法, 都是以 sortedArray 开头的.
 首先进行原始数组的拷贝操作, 然后在拷贝的数组上, 进行 sort 的操作, 然后返回 sorted 的数组.
 */
- (NSArray*) sortedArrayUsingFunction:
(NSComparisonResult(*)(id,id,void*))comparator
                              context: (void*)context
...
- (NSString*) componentsJoinedByString: (NSString*)separator
{
    ...
    if (c > 0)
    {
        ...
        NSUInteger	i;
        /*
         首先进行一次操作, 然后在循环里边进行相同的操作. 这样让代码更加的简洁.
         */
...
/*
 作为一个容器, 要控制 Enumerator 的生成过程.
 Enumerator 一定是特殊的类, 特殊的类中, 包含着迭代的逻辑, 例如顺序, 过滤 等逻辑.
 容器知道自己想要什么样的迭代器, 包装在方法的定义里面, 外界仅仅知道, 传出来一个 NSEnumerator 结果的对象而已.
 */
 ...
 */
/*
 这是 NSArray 对于 Kvc 的特殊处理. 本身, array 里面没有多少数据的.
 Foundation 里面, 表现出的, 返回一个数组, 这个数组是每个对象的 valueforKey 的效果, 是专门在这个函数里面, 进行的实现而已.
 */
                          ...
                          usingBlock: (GSEnumeratorBlock)aBlock
{
    ...
    /*
     enumerate
     实际上, 还是使用了 NSFastEnumeration, 这是一个良好的抽象, 值的在所有需要遍历的地方, 都使用这层抽象.
     */
    {
...
- (void) addObjectsFromArray: (NSArray*)otherArray
{
    ...
    if (c > 0)
    {
        ...
        for (i = 0; i < c; i++)
            (*add)(self, addSel,  (*get)(otherArray, objectAtIndexSel, i));
    }
}
        ...
        if ((c = [otherArray count]) > 0)
        {
            ...
            for (i = 0; i < c; i++)
            {
                (*rem)(self, @selector(removeObject:),
                       (*get)(otherArray, objectAtIndexSel, i));
            }
            RELEASE(otherArray);
        }
    }
...
/*
 最终排序, 还是调用公共的方法.
 这些 GSSortStableConcurrent, GSSortStable 的具体实现不用去看, 和 归并排序, 快速排序还是不同的.
 */
