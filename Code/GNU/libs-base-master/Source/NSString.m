...
(NSCharacterSet *)aSet
{
    // 首先, 用 UTF8 这种统一的方式, 先确定二进制数据来.
    // 这样, 还原的时候, 就能够还原成为对应的 UTF8 的数据来了.
    // 百分号转义的原则很简单. 一个字节分两份, 前面加个 %. 这样, 看到一个百分号, 读后面两个数, 就能还原出原来的那个字节的值了.
    // 通过这种转义之后, URL 里面只会出现, 它认为是安全的值, 其他所有的值, 又都可以使用 %12 这种方式传递.
    ...
    if (data != nil)
    {
        ...
        result = (unsigned char*)NSZoneMalloc(NSDefaultMallocZone(), slen * 3);
        while (spos < slen)
        {
            unichar	c = src[spos++];
            unsigned int	hi;
            ...
            else // if not, then encode it...
            {
                result[dpos++] = '%';
                hi = (c & 0xf0) >> 4;
                result[dpos++] = (hi > 9) ? 'A' + hi - 10 : '0' + hi;
                lo = (c & 0x0f);
                result[dpos++] = (lo > 9) ? 'A' + lo - 10 : '0' + lo;
            }
        }
        s = [[NSString alloc] initWithBytes: result
                                     length: dpos
...
// 总之, 判断是不是绝对路径, 就是在判断, 符不符合绝对路径的规则.
- (BOOL) isAbsolutePath
{
    unichar	c;
    unsigned	pathLength = [self length];
    unsigned	root;
