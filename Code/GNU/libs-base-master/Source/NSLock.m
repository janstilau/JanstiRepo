...
/*
 PV 操作, 都是原子操作, 在 PV 本质上说, 是 test and modify 操作. 通过关中断开中断, 使得在进行原子操作的时候, 操作系统不可以进行线程切换.
 P 操作, 首先将资源值--, 然后判断资源值 <=0, 如果 < 0, 就把当前线程记录到待唤醒队列中, 阻塞该线程, 退出原语操作.
 V 操作, 首先将资源值++, 然后判断资源值是否 <= 0, 如果是 <=0, 就证明有着需要被唤醒的线程, 唤醒第一个被阻塞的线程.
 注意, PV 操作里面没有循环, 是根据线程的阻塞, 停止的当前任务.
 
 所以信号量最重要的, 是阻塞唤醒的机制.
 
 当信号量值为 1 的时候, 只会有一个线程进入临界区, 这就形成了互斥的目的. 在退出临界区之后, 进行 V 操作, 被阻塞线程被唤醒, 继续 lock 之后的任务.
 ...
 但是这种同步, 仅仅达到了有序的目的, 对于共享资源的访问, 还是需要互斥来保证.
 
 condition 可以认为是, 内部含有一个初始值为 0 的信号量. 先加锁, 因为加锁了, 所以可以使用共享的资源进行判断, 当发现条件不允许的情况下, 就进行 wait.
 wait 操作, 首先将 P 操作, 将当前线程加入到待唤醒线程中, 然后放开锁.
 这样, 其他的线程就可以获取到锁, 进行共享资源的操作, 然后进行 signal 操作, 也就是 V 操作. 之前阻塞的线程被唤醒,
 会立马再次加锁, 保证对于共享资源的互斥访问, 然后再次进行判断, 如果不合适, 再次进行 wiat, P 操作, 并放开互斥锁.
 而 broadcast 会将所有等待的线程都唤醒, 一次性把所有阻塞的线程都放入就绪队列, 被唤醒的线程, 首先会尝试加锁, 然后判断条件, 如果不允许的话, 还会进行 wait, 相当于进行 P 操作并释放锁.
...
// 这个操作, 会把所有的被阻塞的线程进行唤醒.
- (void) broadcast
{
    pthread_cond_broadcast(&_condition);
}

// 这个操作, 会让当前线程进入阻塞队列, 并且放开互斥锁. 可以想象, 这是一个原语操作.
- (void) wait
{
