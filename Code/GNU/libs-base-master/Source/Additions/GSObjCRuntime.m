...
GSObjCMakeClass(NSString *name, NSString *superName, NSDictionary *iVars)
{
    ...
    /*
     OC runtime API 暴露出来的, 进行类的创建的接口.
     */
    newClass = objc_allocateClassPair(classSuperClass, classNameCString, 0);
    
    /*
     在类创建完之后, NSDictionary *iVars 里面存储着新类的 ivar 信息.
     这里面只有 name: NSString, age: Int 这样的信息, 具体的 size 信息, 根据类型名进行获取.
     */
    if ([iVars count] > 0)
    {
        ...
        while ((key = [enumerator nextObject]) != nil)
        {
            ...
            // Record actual size
            iVarSize = s;
            if (NO == class_addIvar(newClass, iVarName, iVarSize, iVarAlign, iVarType))
            {
