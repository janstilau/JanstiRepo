...
/*
 runloop 的通知, 调用 dispatch, 做事件的分发操作.
 */
...
/*
 所谓的 schedule, 就是将 stream 放到 runloop 上.
 */
- (void) _schedule
{
...
/*
 _sendEvent, 主动通知 delegate 做后续操纵.
 _events 里面存储的是已经 发送过的时效件.
 */
- (void) _sendEvent: (NSStreamEvent)event delegate: (id)delegate
{
    ...
    {
        if ((_events & event) == 0) // 如果, 还没有发送过 opened, 才进行发送.
        {
    ...
    else if (event == NSStreamEventHasBytesAvailable) // 有内容了
    {
    ...
    else if (event == NSStreamEventHasSpaceAvailable) // 有空间了
    {
        if ((_events & NSStreamEventOpenCompleted) == 0)
        {
            _events |= NSStreamEventOpenCompleted;
    ...
    else if (event == NSStreamEventEndEncountered) // 到头了
    {
...
/*
 _unschedule 就是将 stream 从 runloop 上移除.
 */
- (void) _unschedule
{
    NSMapEnumerator	enumerator;
...
/*
 首先, runloop 通知 stream 可以进行操作了.
 stream 根据当前的状态, sendEvent 到自己的 delegate 里面.
 自己的 delegate, 根据发送过来的 event, 调用 stream 的函数, 进行 get, 或者 send 操作.
 这就是 stream 可以进行这种流式发送的原因.
 */

/*
 DataInput, 已经有了一块 Data 了, 进行逐步的读入操作.
 */
