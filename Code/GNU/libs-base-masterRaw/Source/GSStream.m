...
- (void) addStream: (NSStream*)aStream mode: (NSString*)mode
{
    ...
    // Runloop 添加 stream, 就是把 stream 当做了自己的一个 watcher, 和 mode 绑定.
    [self addEvent: event
              type: type
...
{
    _events &= ~mask;
    // &= ~mask; 这种方式就是其他值保持不变, mash 对应的值, 取反.
}
...
+ (void) initialize
{
    ...
    {
        // GSInputStream 的父类, 是 InputStream, 所以, 这里是使用 GSObjCAddClassBehavior 这种方法, 完成的方法的复用.
...
// read, 就是在消耗 stream 管理的资源, 在这里面, 要做 offset 的管理, 和自己的状态管理.
- (NSInteger) read: (uint8_t *)buffer maxLength: (NSUInteger)len
{
    ...
    // 在读取的过程中, 设置自己没有空间了. 这个值, 会在 dispatch 里面修改. dispatch, 就是到了下一个运行循环了.
    _events &= ~NSStreamEventHasBytesAvailable;
...
// 对于 Data Stream 来说, 每次 Runloop 触发的逻辑就是, 根据自己的状态, 触发事件.
// 发送事件, 就是通知代理, 然后代理, 一点点通过 stream 读取数据.
// 本身 Data 在 Data Stream 里面, 是完成的读取的.
// 外界使用起来, 是使用 stream 的方式. 这样, 外界就可以使用 stream 的接口, 完成统一的处理.
- (void) _dispatch
{
...
// GSBufferOutputStream 的功能是, 外界写数据到该 stream 里面去.
...
// 把 Buffer 里面的内容, 写到 stream 里面去, buffer 最大长度是 len.
...
// GSDataOutputStream 就是一个可变的 data, 不会出现到头的情况.
