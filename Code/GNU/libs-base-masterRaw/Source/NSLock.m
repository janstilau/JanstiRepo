...
{
    [_condition lock];
    // 其实, NSCondition 使用的时候, 就应该是这种模式.
    // [_condition wait] 被唤醒了, 一定是 condition 里面的 lock 重新加锁. 这样, while 后面的 predicate 就在一个线程安全的环境了.
    // conditionLock 仅仅是把这个 predicate, 变为了 int 的比较而已.
