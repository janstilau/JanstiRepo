...
// 在这里, 将 mode 对应的 observser 的回调进行调用.
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef runloopRef,
                            CFStringRef modeName,
                            CFTimeInterval seconds,
                            Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
    if (__CFRunLoopIsDeallocating(runloopRef)) return kCFRunLoopRunFinished;
    __CFRunLoopLock(runloopRef);
    {
        CFRunLoopModeRef currentMode = __CFRunLoopFindMode(runloopRef, modeName, false);
        if (NULL == currentMode || __CFRunLoopModeIsEmpty(runloopRef, currentMode, runloopRef->_currentMode)) {
            Boolean did = false;
            if (currentMode) __CFRunLoopModeUnlock(currentMode);
            __CFRunLoopUnlock(runloopRef);
            return did ? kCFRunLoopRunHandledSource : kCFRunLoopRunFinished;
        }
        volatile _per_run_data *previousPerRun = __CFRunLoopPushPerRunData(runloopRef);
        CFRunLoopModeRef previousMode = runloopRef->_currentMode;
        runloopRef->_currentMode = currentMode;
        int32_t result = kCFRunLoopRunFinished;
        
        // 在进入 RunLoop 之前, 通知 observer.
        if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(runloopRef, currentMode, kCFRunLoopEntry);
        // 然后进入真正的 Runloop 的过程
        result = __CFRunLoopRun(runloopRef, currentMode, seconds, returnAfterSourceHandled, previousMode);
        // 在退出 RunLoop 之后, 通知 observer.
        if (currentMode->_observerMask & kCFRunLoopExit ) __CFRunLoopDoObservers(runloopRef, currentMode, kCFRunLoopExit);
        
        __CFRunLoopModeUnlock(currentMode);
        __CFRunLoopPopPerRunData(runloopRef, previousPerRun);
        runloopRef->_currentMode = previousMode;
    }
    __CFRunLoopUnlock(runloopRef);
    return result;
}
