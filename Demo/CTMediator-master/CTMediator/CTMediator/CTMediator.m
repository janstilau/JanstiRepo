...
// 这里, 对于 url 的处理, 使用了一个固定的模式, 就是 url 的 host 代表的是 target, path 代表的就是 action.
// query 里面的 path, 当做了 action 
...
- (id)performTarget:(NSString *)targetName action:(NSString *)actionName params:(NSDictionary *)params shouldCacheTarget:(BOOL)shouldCacheTarget {
    ...
    // 这里, 会取到 target 字符串生成的 target 对象.
    ...
    // 如果, target 可以相应对应的请求, 那么就去调用相应的方法
...
// 这里, 如果有保底的 Target_NoTargetAction 类, 那么就将没有相应的一些信息交给他进行处理.
...
// 这里, 调用方法使用了 方法签名, 已经 INVOCATION 这个类 来实现.
- (id)safePerformAction:(SEL)action target:(NSObject *)target params:(NSDictionary *)params
{
    ...
    // 如果, 这个 Action 是没有返回值的, 那么直接构造没有返回值的 NSInvocation, 将 target, action, params 填入, invoke.
    ...
    // 以下都是对于基本数据类型的返回值, 生成对应的 result, 获取到对应类型的数据, 返回.
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    // 如果, return type 不是基本数据类型, 那么就直接用 performSelector
