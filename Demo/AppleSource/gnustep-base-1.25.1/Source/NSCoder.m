...
- (void) encodeArrayOfObjCType: (const char*)type // 数组内元素的类型. 通过这个, 可以确定数组的长度.
                         count: (int)count // 数组单位长度
                            at: (const void*)array
{
    unsigned	i;
    ...
    imp = [self methodForSelector: @selector(encodeValueOfObjCType:at:)]; // 这里,encodeValueOfObjCType这个函数必须子类复写.
