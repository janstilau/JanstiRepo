...
- (BOOL) setObject: (id)anObject forKey: (NSString*)aKey
{
    if (nil == anObject) // 去除值.
    {
        ...
        if ([added member: aKey]) // If anObject is in set, return it (the copy in the set). 这里, 同 set 存储变化.
        {
...
- (BOOL) synchronize
{
    ...
    NS_DURING
    {
        ...
        if (YES == [mgr isReadableFileAtPath: path])
        {
            ...
            if (nil != data) // 取出 data
            {
                id	defaultsPropertylist;
                
                defaultsPropertylist = [NSPropertyListSerialization
                     propertyListWithData: data
                     options: NSPropertyListImmutable
                     format: 0
                     error: 0];
                if ([defaultsPropertylist isKindOfClass: [NSDictionary class]])
                {
                    diskDict = AUTORELEASE([defaultsPropertylist mutableCopy]); // 将 propertyList 变为 NSDicitonaryM
                }
            }
        }
        if (nil == diskDict)
        {
            diskDict = [NSMutableDictionary dictionary];
        }
        loaded = YES;
        
        if (NO == [contents isEqual: diskDict])
        {
            defaultsChanged = YES;
            // remove, added, mofified 都是缓存数据, 只有到了 synchronize 的时候, 才会进行真正的变化
            if (YES == hasLocalChanges)
            {
                ...
                while (nil != (k = [e nextObject]))
                {
                    [diskDict removeObjectForKey: k];
                }
                e = [added objectEnumerator];
                while (nil != (k = [e nextObject]))
                {
                    [diskDict setObject: [contents objectForKey: k] forKey: k];
                }
