...
/*
 面向对象的好处, 就在于它可以把操作的细节, 封装到自己的内部.
 NSLock 里面, 是直接操作 pthread_mutex 的函数. 由这些函数, 去和操作系统打交道.
 各种对象级别的操作, 都可以通过这些进行模拟.
 */
...
/*
 lockBeforeDate 的实现, 就是不断的调用 pthread_mutex_trylock 这个方法, 如果没有获取到锁的话, 就直接退出线程了.
 sched_yield 也是一个底层的 API, 就是告诉操作系统, 可以调度其他的县城了.
 循环的终止条件, 要么是获取到锁了, 要么就是时间到了, 退出循环.
 */
- (BOOL) lockBeforeDate: (NSDate*)limit
{
    do
...
/*
 这个类和 NSLock 没有任何的区别, 不过是初始化的时候, 用 attr_recursive 来进行.
 递归锁指的是, 只要是当前的线程, 都可以重复的进行锁的操作, 当然, 相应的, 就是在退出临界区的时候, 要进行 unlock 的操作. 当最后一次 unlock 的时候, 才会释放锁.
 */
...
{
    /*
     // 这个函数, 会释放自己当前已经捕获的 mutex, 然后陷入🔐中, 直到其他的线程进行 singnal, 或者 broadCast 的唤醒. 并且, 唤醒之后, 还要接着重新获取 mutex 才能继续后面的操作;.
     */
...
- (id) initWithCondition: (NSInteger)value
{
    ...
    {
        /*
         这里, 这个写法不好, 初始化就初始化, 为什么要放在 if 判断里面.
         */
...
/*
    wait 之后, 会被 signal, 或者 broadcast 唤醒, 但是唤醒之后, 如果发现, value 和自己当前的不一样, 继续进行等待.
    这里, value 是保存在栈上面的, 是每个执行线程不一样的. 而 _condition_value 是保存在ConditionLock 里面的, 是各个执行线程共享的.
    所以, 各个线程的等待条件, 是通过参数进行的分别.
 */
