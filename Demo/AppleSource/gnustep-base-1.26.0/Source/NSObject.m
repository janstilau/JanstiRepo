...
static id objc_retain_fast_np_internal(id anObject)
{
    ...
    pthread_mutex_lock(theLock);
// 这里, 是 GNU 的 retainCount 的修改, 也就是在对象的头部空间开辟一段内存专门存储 retainCount 的信息.
...
/**
 ...
 就如同, == 的重载一样, 有些操作是基本方法, 这些操作, 会在各个地方用到, 比如, 容器里面判断位置, 容器里面判断相等.
 OC 里面没有操作符重载这回事, 所以, 只能显示的重写 isEqual 方法, 然后修改了这个方法之后, 就能在用到的所有位置进行自定义的操作.
 而 对于可以使用操作符重载的语言来说, == 的判断是更好的方式. 因为在不了解 OC 的原理时, 确实是有很多的程序员用 == 进行了相等性的判断, 这就代表着, == 判断相等, 其实是更加符合直觉的.
 swift 里面, isEqual 的判断, 转移到了 == 操作符上, 而同一性的判断, 则用 === 这个操作符进行了替换.
 ...
 * <br />Raises NSInvalidArgumentException if given a null selector.
 SEL 是一个结构, 里面是一些方法相关的信息, 这个数据结构不透明, 不过可以确认的是, 第一个成员必然是一个字符串指针, 因为可以直接用 %s 打印.
 每个类里面, 可以有着相同的 SEL , 但是每个 SEL 对应的 IMP 是不同的函数.
 通过 SEL, 可以将方法调用, 变为一个动态的过程, 而不是直接的函数调用地址的链接.
 这样, 就可以完成方法的运行时的绑定工作, 也为 OC 动态添加方法, 提供了实现的可能性.
 本质上, 这还是一个元信息编程的使用.
 */
- (id) performSelector: (SEL)aSelector
{
     ...
     * use forwarding callbacks to get better type information.
     */
    // 这里和 Runtime 的源码有出入, runtime 这里调用了 objc_msg_send. 而这里是直接取得 IMP 调用给了.
 ...
 * in MacOS-X compatibility more, or returns NO otherwise.
 ...
 class_respondsToSelector 的内部是, 查找 IMP, 如果可以查找到, 就返回 YES, 否则就返回 NO.
 也就是说, 是通过 方法列表的 hash 查找得到的最终的结果.
 */
