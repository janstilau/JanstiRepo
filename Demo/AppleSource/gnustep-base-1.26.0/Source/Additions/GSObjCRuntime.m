 ...
 * 通过类对象, 获取 name 对应的属性的类型, 大小, 已经偏移量.
 */
BOOL
             ...
             const char *type, unsigned size, int offset)
{
    ...
    else
    {
        ...
        // 这里的套路也是一样的, 根据 type 定义相应类型的对象, 然后通过 SEL, 或者指针的偏移, 获取到对应的量.
        switch (*type)
        {
            case _C_ID:
            case _C_CLASS:
             ...
             const char *type, unsigned size, int offset)
{
    ...
    { // 下面的套路都一样, 如果有 SEL, 就调用SEL, 如果没有, 就根据类型和偏移值进行直接的复制. 根据类型定义相对应的指针, 然后通过指针进行赋值.
        switch (*type)
        {
            case _C_ID:
            case _C_CLASS:
