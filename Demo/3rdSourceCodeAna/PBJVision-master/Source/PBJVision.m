...
PBJMediaWriterDelegate>
{
    // AV
    /*
     https://www.jianshu.com/p/94f11abe13a7
...
// 将这几个对于 block 的调度, 用方法进行封装, 将 queue 和 block 的关系, 进行抽象.
...
{
    // beginConfiguration, commitConfiguration 用来进行原子化操作.  on a running session into an atomic update.
    // 初始化的时候, 可以进行设置, 但是在一个已经 run 的 session 上操作, 就应该用 begin, commit.
...
- (void)startVideoCapture
{
    ...
    [self _enqueueBlockOnCaptureVideoQueue:^{
        ...
        // 点击开始录制的时候, 才会开启一个 PBJMediaWriter.
        if (_mediaWriter) {
...
- (BOOL)_setupMediaWriterAudioInputWithSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    NSDictionary *audioCompressionSettings = @{ AVFormatIDKey : @(kAudioFormatMPEG4AAC),
                                                ...
                                                AVEncoderBitRateKey : @(_audioBitRate), // [NSNumber numberWithInteger:128000]
                                                AVChannelLayoutKey : currentChannelLayoutData, // [NSNumber numberWithInteger:44100]
    };
