...
static void methodizeClass(Class cls)
{
    ...
    // 首先, 是从 rot 里面, 添加信息. 从这里可以看到, 类的原始信息, 放到列表的最后位置了.
...
static Class realizeClass(Class cls)
{
    ...
    // data 返回的是 rwt, 但这是在初始化的过程中, 所以应该是类内部的数据, 就是 rot
    ...
    // 实现父类, 以及元类对象, 这两个对象要在下面用到
    supercls = realizeClass(remapClass(cls->superclass));
    metacls = realizeClass(remapClass(cls->ISA()));
    
    // 设置 superclass 和 isa 的指向
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    // 如果类需要初始化,(编码人员自定义的 initialize 方法, 在这里面可以做一些类的自定义初始化工作.) 就调用_class_initialize进行初始化.
    // 从这里我们看到, 平时所有, 当一个类第一次被用到的时候, 调用 initialize 方法, 而这第一次被用到, 是发生在消息转发的过程之中.
 ...
 retry:    
    ...
    {
        // 在自己类的内部从 methodlist 中寻找实现
        Method meth = getMethodNoSuper_nolock(cls, sel);
        if (meth) {
            // 如果找到了, 就将这个实现添加到接受者类的 cache_t 中
    ...
    // 如果本类没有实现, 就从父类开始, 从上层类中寻找实现.
    {
             ...
             curClass = curClass->superclass)
        {
            ...
            // 父类的缓存中没有, 就存父类的 method_list 中寻找.
            ...
            // log_and_fill_cache 中, cls 都是消息接受者的类对象, 也就是只要找到实现, 都要在这个类的 chache_t 中进行一次备份
        }
    }

    // No implementation found. Try method resolver once.
    // 进入消息解析流程. 然后重新进行一次上面的检索操作.
